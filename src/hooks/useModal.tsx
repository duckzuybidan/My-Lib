import { IModalData, IModalState } from "@/types/modal-type";
import { create } from "zustand";



const store = create<IModalState>((set) => ({
  isOpen: false,
  type: "",
  data: undefined,
  onSubmit: () => { },
 ...
 * - `onSubmit`: A callback function triggered when the modal's submit action is executed
 * 
 ...
 * Example usage:
 * ```tsx
 * const { isOpen, openModal, closeModal, type, data, onSubmit, setData } = useModal<'DELETE'>();
 * 
 * // To open the modal
 * openModal({
 *   type: 'DELETE',
...
const useModal = <T extends keyof IModalData>() => {
  const { data, setData, type, isOpen, closeModal, openModal, onSubmit } = store()
